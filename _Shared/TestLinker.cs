...
{
  public ITag SubLinkWithIndicator(string paramName, string value, string title, bool forceSet = false) {
    var pParams = CmsContext.Page.Parameters;
    var existingValue = pParams[paramName];
    var isSet = forceSet || Text.Has(existingValue) && Text.Has(value)
